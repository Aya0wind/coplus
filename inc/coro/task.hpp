...
namespace coplus {
    ...
    struct task {
        ...
        explicit task(co_handle<promise_type> handle) : handle(handle),id(id_generator::next_id()) {}
        task(task const &) = delete;
        task() : handle(nullptr),id(id_generator::next_id()) {}
        task(task &&other) noexcept: handle(other.handle),id(other.id) {other.handle = nullptr;}
        ~task() {
            if (handle) {
                handle.destroy();
            }
    ...
    struct task<void> {
        ...
        task() : handle(nullptr),id(id_generator::next_id())  {}
        task(task const &) = delete;
        explicit task(co_handle handle):handle(handle),id(id_generator::next_id()) {}
        task(task &&other) noexcept: handle(other.handle),id(other.id){other.handle = nullptr;}
        void resume() {
            handle.resume();
        }
